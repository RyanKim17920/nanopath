...
Recent H100 timings from the untouched baselines after the PathoBench-style retile and no-crop patch transform. DINOv2-random, DINOv2-G, and OpenMidnight were run directly on the 11-probe suite; the other probe-wall rows subtract the removed response task from their earlier full reruns because no remaining task changed. Wall time varies with concurrent jobs and OS page cache.
...
| `crc_survival` | 152.2s | 173.7s | 261.3s | 266.8s | 263.0s | 708.2s |
| `pathorob` | 34.5s | 28.3s | 72.3s | 75.6s | 74.5s | 198.4s |
| **probe wall** | **641.3s** | **707.2s** | **1080.7s** | **1160.5s** | **1076.0s** | **2753.9s** |
